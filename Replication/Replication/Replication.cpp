...
std::unordered_map<int, std::mutex> queueMtxsBackup;
std::mutex backupCMutex;
std::condition_variable backupConditionClient;
std::queue<SOCKET> backupConnections;
...
DWORD WINAPI handleProcesses(LPVOID lparm) {
    ...
    while (true) {

        {
            ...
            dataCondition.wait(lock, [] { return !socketQueue.empty() || stopFlag == true; });
      ...
      //  printfP("Waiting for data...\t\t[\tPress enter for menu!\t]", nullptr,totalMessages.load(), totalBackup.load());
        while (true) {
            ...
            if (currentId == data.deviceId) {                   
                ...
                if (SendMessageQ(copyServiceSocket,data,get_queue_for_key(&messageQueues, data.deviceId), queueMtxs[data.deviceId])) {
...
DWORD WINAPI handleBackupToClient(LPVOID lpParam) {
    ...
    while (true) {
        ...
        backupConditionClient.wait(lock, [] { return !backupConnections.empty() || stopFlag == true; });

        printf("\n\t\t\tBackup thread started, searching backup...");
            
        if (stopFlag == false) {
                socket = backupConnections.front();
                backupConnections.pop();
            ...
            {
                std::unique_lock<std::mutex> lock(queueMtxsBackup[currentBackupChoice]);
...
DWORD WINAPI getMessagesThread(LPVOID lpParam) {
    ...
    while (!stopFlag) {
        ...
        while (!stop) {
            ReceiveAndFilterMessages(stopFlag, m, backupRequest, totalBackup, stop, localCounter, messageQueuesBackup, queueMtxsBackup[currentBackupChoice]);
            }
        ...
        if (clientSocket != INVALID_SOCKET)
        {
            if (pars.id == 1) {
            ...
            else if (pars.id == 2) {
                {
                    ...
                    std::unique_lock<std::mutex> lock(backupCMutex);
                    backupConnections.push(clientSocket);
                   // backupConditionClient.notify_one();
                }
            }
...
int starting() {
    ...
    while (!socketQueue.empty()) {
        if (socketQueue.front()!= INVALID_SOCKET) {
